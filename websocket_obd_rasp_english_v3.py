...
from utils.trip_log import init_trip_log, save_row_dynamic
...
async def _startup():
    global _start_monotonic, LLM, ORCH, TRIP_START_ISO, TRIP_LOG_FILE
    _start_monotonic = time.monotonic()

    await init_alerts_index()

    TRIP_START_ISO = datetime.now(timezone.utc).isoformat(timespec="seconds").replace(":", "-").replace("+00:00", "Z")
    TRIP_LOG_FILE = TRIP_DIR / f"trip_{TRIP_START_ISO}.csv"

    is_ready = await wait_llm_ready(base_url="http://127.0.0.1:8080/v1", total_timeout_s=10.0, interval_s=0.5)

    # Initialize runtime HTTP - llama.cpp
    # Ex.: ./server -m /path/model.gguf -c 1024 -t 4 --port 8080 --host 127.0.0.1
    if is_ready:
        ...
        LLM = None
    ORCH = Orchestrator(llm=LLM)  # se quiser sem LLM: Orchestrator(llm=None)

    trip_path = init_trip_log(base_dir=os.getenv("TRIP_LOG_DIR","./trips"))
    print(f"[trip] logging em: {trip_path}")
